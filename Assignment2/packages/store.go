...
func price(w http.ResponseWriter, r *http.Request) {
	res, _ := template.ParseFiles("client/price.html")
	//var mydatabase allinone
	//mydatabase.Userdata = temp
	//mydatabase.Items = GetItems()
	res.Execute(w, nil)
}
...
func Market(user user.User) {
	...
	//http.HandleFunc("/filter", Filter) // Add this line to register the Filter function
